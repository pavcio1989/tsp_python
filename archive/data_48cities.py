...
def read_city_names():
    with open("../data/input/brazil/city_names.txt") as file:
        f = file.readlines()
...
def read_lat_lon():
    df = pd.read_csv("../data/input/brazil/coord.txt", header=0, sep=" ")
...
def read_distance_matrix():
    with open("../data/input/brazil/distance_matrix.txt") as file3:
        f3 = file3.readlines()
    dist_mat = [x.replace("\n", "").split() for x in f3]
